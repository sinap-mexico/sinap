...
export async function POST() {
  try {
    ...
    if (existingClinic && existingClinic.doctors.length > 0) {
      // Demo data already seeded — return existing clinic info
      ...
      for (const msg of conv.messages) {
        await db.message.upsert({
          ...
          create: {
            ...
            content: msg.content, agentName: (msg as any).agentName,
            aiGenerated: (msg as any).aiGenerated ?? false, confidence: (msg as any).confidence,
    ...
    return NextResponse.json({
      ...
      summary: {
        clinic: 1, doctors: 2, staff: staffData.length, patients: patientsData.length,
        appointments: appointmentsData.length, invoices: invoicesData.length,
        conversations: conversationsData.length,
        messages: conversationsData.reduce((sum, c) => sum + c.messages.length, 0),
        featureFlags: featureFlags.length, soapNotes: 2, eventBus: eventsData.length,
      },
    })
  } catch (error: unknown) {
