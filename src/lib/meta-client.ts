...
// Handles: text messages, template messages, media messages,
...
export interface MetaSendResponse {
  ...
  whatsappId: string  // recipient phone
}
...
class MetaClient {
  ...
  // ─── Internal: send request to Meta API ─────────────────
  private async sendRequest(body: Record<string, unknown>): Promise<MetaSendResponse> {
    ...
    try {
      ...
      // Meta response: { messages: [{ id: "wamid.xxx" }], contacts: [{ wa_id: "52xxx" }] }
...
// ─── Helper: Get Meta config for a clinic from DB ──────────
...
export async function validateMetaToken(accessToken: string): Promise<{ valid: boolean; businessName?: string; wabaId?: string }> {
  const controller = new AbortController()
  const timeout = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS)

  try {
    const url = `${META_BASE_URL}/me?access_token=${encodeURIComponent(accessToken)}`
