...
export function AgendaCalendar() {
  ...
  const getAppointmentsForDate = useCallback((date: Date) => {
    // For demo, show all mock appointments on the current date view
    const today = new Date()
    const isToday = date.toDateString() === today.toDateString()
    if (isToday) {
      return appointmentList
    }
    // For other days in week view, show fewer appointments
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-base font-medium tracking-[-0.03em] text-[#2C2C2A] min-w-[200px] text-center">
            {view === 'day' ? formatDate(currentDate) : `${formatDate(weekDays[0])} — ${formatDate(weekDays[5])}`}
          </h2>
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={navigateNext}>
            <ChevronRight className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="sm" className="h-7 text-xs border-[#1D9E75] text-[#1D9E75]" onClick={goToday}>
            Hoy
          </Button>
        </div>
          ...
          {clinicMode === 'clinic' && (
            <Select value={selectedDoctor} onValueChange={setSelectedDoctor}>
              ...
              <SelectContent>
                ...
                {staffMembers.filter(s => s.specialty !== 'Recepción').map(s => (
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                className="text-sm bg-[#F1EFE8] border-[#E1F5EE] min-h-[60px]"
                value={newNotes}
              ...
              {selectedAppointment.status !== 'cancelled' && (
                <div className="flex gap-2">
                  {selectedAppointment.status !== 'confirmed' && (
                    <Button
                      className="flex-1 bg-[#1D9E75] hover:bg-[#1D9E75]/90 text-white text-sm h-9"
